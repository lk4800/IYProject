...
<body>
	...
	<div class="board_write_wrap">
		...
		<span id="board_fb_num">${fbCont.fb_num}</span>
		<input placeholder="제목을 입력해 주세요." id="board_title" style="height: 40px;" value="${fbCont.fb_title}" />
		<textarea id="board_summernote">${fbCont.fb_cont}</textarea>
	
		<div id="board_write_btmbtn">
			<button id="board_write_listbtn" onclick="location='IY_board_flist'">목록</button>		
	...
	<script>
		...
		function check() {
			if ($.trim($('#board_title').val()) == '') {
				...
				const boardInfo={
						fb_num:$("#board_fb_num").text(),
						fb_title:$("#board_title").val(),
		    		fb_cont:$("#board_summernote").val(),
		    		fb_img_url:url,
